...
class InterpretersIntegrationSpec extends AnyFlatSpec with Matchers with JsonMatchers {
  ...
  it should "create a simple system" in {
    ...
    SkuberConverter(systemInterpreter).toMetaAndJsValue foreach {
      case (ShortMeta(_, "Service", _, "app-one"), json) =>
...
"""))
      ...
      case (ShortMeta(_, "Deployment", _, "app-one"), json) =>
        json should matchJsonString(yamlToJson("""
---
apiVersion: apps/v1
kind: Deployment
...
"""))
      ...
      case (ShortMeta(_, "Service", _, "app-two"), json) =>
        json should matchJsonString(yamlToJson("""
---
apiVersion: v1
kind: Service
...
"""))
      ...
      case (ShortMeta(_, "Deployment", _, "app-two"), json) =>
        json should matchJsonString(yamlToJson("""
---
kind: Deployment
apiVersion: apps/v1
...
spec:
  ...
  template:
    ...
    spec:
      ...
      dnsPolicy: ClusterFirst
"""))
      case (ShortMeta(_, "Namespace", _, "test"), json) =>
        json should matchJsonString(yamlToJson("""
---
kind: Namespace
apiVersion: v1
